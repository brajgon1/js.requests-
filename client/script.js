...
sayHelloButton.addEventListener("mouseover", changeBtnColors);
...
sayHelloButton.addEventListener("mouseout", changeBackgroundColor);
...
// NEED HELP WITH THIS ONE// document.getElementById('say-hello-button');
...
const repeatMyParam = () => {
  axios.get('http://localhost:3000/repeat/Howdy').then((response) => {
...
const query = () => {
    axios.get('http://localhost:3000/query-test?input=cats-are-good-for-us&dogs-are-just-fine&pets-are-the-best').then((response) => {
        console.log(response.data);
      });
}
...
// it said "you sent an empty query!" in the console 
    2: Send more than 1 query on the URL -- what happened? 
// it said "you sentt more than 1 query!" in the console     
*/
